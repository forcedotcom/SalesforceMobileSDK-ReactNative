...
When running the full test suite (e.g., in CI), all tests in a suite run together via the "Run All" button. This is much faster.

**Usage**: Set environment variable `useBatchExecution=true`:
- **Xcode**: Edit Scheme → Test → Arguments → Environment Variables
- **Command line**: `xcodebuild test ... useBatchExecution=true`

For more details on batch vs individual execution, see `docs/ios-tests/README.md`.
