...
export default function TestApp() {
  ...
  return (
    <ScrollView style={styles.container} testID="testList">
      ...
      <TouchableOpacity style={styles.runAllButton} testID="runAll" onPress={handleRunAll}>
      ...
      {Object.entries(suites).map(([suiteName, suite]) => (
        <View key={suiteName} style={styles.suite}>
          <View style={styles.suiteHeader}>
            ...
            <TouchableOpacity testID={`runSuite_${suiteName}`} onPress={() => handleRunSuite(suiteName)}>
            ...
            return (
              <View key={name} style={styles.testRow}>
                <View style={styles.testInfo}>
                  {status === STATUS_PASS && <Text testID={`result_${name}_pass`} style={styles.pass}>✓</Text>}
                  {status === STATUS_FAIL && <Text testID={`result_${name}_fail`} style={styles.fail}>✗</Text>}
